...
namespace Sphynx::ReflectionGenerator {
	...
	public:
		Parser(const std::vector<Token>& tokens, const std::filesystem::path& filepath, std::vector<ComponentReflectionInfo>& outComponents)
			: m_Tokens(tokens), m_Components(outComponents), m_Filepath(filepath)
		{
			...
			while (!Finished()) {
				if (Match(TokenType::COMPONENT_PROP)) {
					...
					ComponentReflectionInfo& componentInfo = m_Components.emplace_back();
					ParseDefinition(componentInfo);
					componentInfo.Filepath = m_Filepath;
				}
...
//				}
		...
		std::string GetErrorMessagee() const { return m_ErrorMessage + " at " + m_Filepath.string() + " in line " + std::to_string(m_TokenIter->Line) + ":" + std::to_string(m_TokenIter->Column); }
