...
int main(int argc, const char** argv) {
	...
	if (!IsDebuggerPresent()) {
#endif
		if (argc != 2) {
			std::cout << "usage: [EngineDir]\n";
			std::cin.get();
			return 0;
		}

		std::string engineDir = argv[1];
		std::erase(engineDir, '\"');
		...
		for (const auto& directoryEntry : std::filesystem::recursive_directory_iterator(std::filesystem::current_path())) {
		...
		std::filesystem::path generatedFolder = std::filesystem::current_path() / L"generated";
